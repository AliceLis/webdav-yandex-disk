...
namespace WebDAV {
	...
	inline ClientImpl * GetImpl(Client * ptr) {
		return (ClientImpl *)ptr;
	}

	inline const ClientImpl * GetImpl(const Client * ptr) {
		return (const ClientImpl *)ptr;
	}
	...
	void ClientImpl::init() const noexcept {
		curl_global_init(CURL_GLOBAL_DEFAULT);
	}

	dict_t ClientImpl::options() const noexcept {
		return dict_t
		{
	...
	unsigned long long Client::free_size() const noexcept {
		Header header = {
			"Accept: */*",
			"Depth: 0",
	...
	bool Client::check(const std::string & remote_resource) const noexcept {
	...
	dict_t Client::info(const std::string & remote_resource) const noexcept {
		auto clientImpl = GetImpl(this);
		auto root_urn = Urn(clientImpl->webdav_root, true);
	...
	bool Client::is_dir(const std::string & remote_resource) const noexcept {
	...
	strings_t Client::list(const std::string & remote_directory) const noexcept {
		auto clientImpl = GetImpl(this);
	...
	bool Client::download(
		...
		auto clientImpl = GetImpl(this);
		auto check = clientImpl->sync_download(remote_file, local_file, nullptr, progress);
		return check;
	}
	...
	Client::async_download(
		...
		std::thread downloading([&]() {
				clientImpl->sync_download(remote_file, local_file, callback, progress);
			});
		downloading.detach();
	}
	...
	bool Client::create_directory(const std::string & remote_directory, bool recursive) const noexcept {
		auto clientImpl = GetImpl(this);
		bool is_existed = this->check(remote_directory);
		if (is_existed)
			return true;
	...
	bool Client::move(const std::string & remote_source_resource, const std::string & remote_destination_resource) const noexcept {
		auto clientImpl = GetImpl(this);
	...
	bool Client::copy(const std::string & remote_source_resource, const std::string & remote_destination_resource) const noexcept {
		auto clientImpl = GetImpl(this);
		bool is_existed = this->check(remote_source_resource);
		if (!is_existed)
			return false;
	...
	Client::async_upload(
		...
		std::thread uploading([&]() {
				clientImpl->sync_upload(remote_file, local_file, callback, progress);
			});
		uploading.detach();
	}
	...
	bool Client::clean(const std::string & remote_resource) const noexcept {
		auto clientImpl = GetImpl(this);
